...
\tikzstyle{startstop} = [rectangle, rounded corners, minimum width=3cm, minimum height=1cm, align=center, draw=black]
\tikzstyle{io} = [trapezium, trapezium left angle=70, trapezium right angle=110, minimum width=3cm, minimum height=1cm, align=center, draw=black]
\tikzstyle{process} = [rectangle, minimum width=3cm, minimum height=1cm, align=center, draw=black]
\tikzstyle{decision} = [diamond, minimum width=3cm, minimum height=1cm, align=center, draw=black]
...
\begin{tikzpicture}[node distance=2cm]
  ...
  \node (argsin) [io, below of=start] {Read arguments};
  \node (filesremaining) [decision, below of=argsin, yshift=-1cm] {Are there files\\remaining to\\be processed?};
  \node (readsrcfile) [io, right of=filesremaining, xshift=3cm] {Read input file};
  \node (parse) [process, below of=readsrcfile] {Parse source};
  \node (typecheck) [process, below of=parse] {Typecheck AST};
  \node (toir) [process, below of=typecheck] {Convert AST to IR};
  \node (toasm) [process, below of=toir] {Generate assemly};
  \node (buildingobjs) [decision, below of=toasm, yshift=-1cm] {Are we building\\object files?};
  \node (assemble) [process, below of=buildingobjs, yshift=-1cm] {Assemble object};
  \node (linkingbinary) [decision, below of=filesremaining, xshift=-3cm, yshift=-7cm] {Are we linking\\a final binary?};
  \node (link) [process, below of=linkingbinary,yshift=-0.5cm, xshift=-3cm] {Link binary};
  \node (end) [startstop, below of=link, xshift=3cm] {End};
  \draw [arrow] (start) -- (argsin);
  \draw [arrow] (argsin) -- (filesremaining);
  \draw [arrow] (filesremaining) -- node[anchor=south] {yes} (readsrcfile);
  \draw [arrow] (readsrcfile) -- (parse);
  \draw [arrow] (parse) -- (typecheck);
  \draw [arrow] (typecheck) -- (toir);
  \draw [arrow] (toir) -- (toasm);
  \draw [arrow] (toasm) -- (buildingobjs);
  \draw [arrow] (buildingobjs) -- node[anchor=west] {yes} (assemble);
  \draw [arrow] (buildingobjs) -| node[anchor=south, xshift=3cm] {no} (filesremaining);
  \draw [arrow] (assemble) -| (filesremaining);
  \draw [arrow] (filesremaining) -| node[anchor=south] {no} (linkingbinary);
  \draw [arrow] (linkingbinary) -- node[anchor=west] {no} (end);
  \draw [arrow] (linkingbinary) -| node[anchor=south] {yes} (link);
  \draw [arrow] (link) |- (end);
\end{tikzpicture}
